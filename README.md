...
# 自动发布并优先复用已有标签页（减少有窗口模式下切到前台）
python scripts/publish_pipeline.py --reuse-existing-tab --auto-publish \
    --title "文章标题" \
    --content "文章正文" \
    --image-urls "https://example.com/image.jpg"
...
# WSL/远程 CDP + Windows/UNC 路径发布时可跳过本地文件预校验
python scripts/publish_pipeline.py --headless --auto-publish \
    --title "文章标题" \
    --content "文章正文" \
...
# 使用指定账号发布
python scripts/publish_pipeline.py --account myaccount --headless --auto-publish \
...
选项:
  ...
  --account NAME         指定账号
  --auto-publish         用户确认后点击发布；实际发布必须显式提供
  --preview              显式预览：仅填充内容，不点击发布（默认行为）
```
...
python scripts/cdp_publish.py --reuse-existing-tab fill --title "标题" --content "正文" --images img.jpg
...
中心。顶部展示最新发布笔记的点赞、收藏、评论、转发、观看与曝光；汇总区展示总点赞、
总收藏、总点赞和收藏（两者合计）以及当前粉丝数，并保留按“点赞 + 收藏”排序的表现靠前笔记，显示点赞、收藏、转发与观看。刷新会遍历
全部分页并校验总条数，同时把账户总数写入配套 JSON；失败时保留上一份快照。

```bash
python scripts/xhs_stats_dashboard.py
python scripts/xhs_stats_dashboard.py --self-test
python scripts/xhs_stats_dashboard.py --refresh-only
```

界面仅展示统计和打开 CSV，不暴露发布或互动命令。
...
## Stars
[![Star history](assets/star-history.svg)](https://github.com/xcbbdg1-maker/post-to-xhs/stargazers)

## 致谢
本仓库基于 MIT 许可延续并改进以下项目，保留原始许可证署名：

- [Angiin/Post-to-xhs](https://github.com/Angiin/Post-to-xhs)
