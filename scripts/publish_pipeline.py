...
Single CLI entry point that orchestrates:
  chrome_launcher → login check → image/video download → form fill → preview (default)
  Publishing only happens when --auto-publish is explicitly supplied.

Usage:
    # Safe default: fill the form for manual review without clicking publish
...
MAX_WEIGHTED_TITLE_LENGTH = 38
CHINESE_PUNCTUATION = frozenset("“”‘’…—·")


def _weighted_title_length(title: str) -> int:
    """Return title length using Xiaohongshu's CJK weighting convention."""

    def is_cjk_or_chinese_punctuation(character: str) -> bool:
        codepoint = ord(character)
        return (
            character in CHINESE_PUNCTUATION
            ...
            or 0xFF00 <= codepoint <= 0xFFEF  # Full-width punctuation/forms
        )

    return sum(2 if is_cjk_or_chinese_punctuation(char) else 1 for char in title)


def _should_publish(auto_publish: bool, preview: bool) -> bool:
    """Return True only for an explicit, non-preview publish opt-in."""
    return bool(auto_publish and not preview)
...
def main():
    parser = argparse.ArgumentParser(
        description=(
            "Xiaohongshu publish pipeline. Safe default: fill the form for "
            "preview; publishing requires explicit --auto-publish."
        )
    )
    ...
    # Publish mode: preview is the safe default; publishing requires explicit opt-in.
    ...
    publish_mode_group.add_argument(
        ...
        default=False,
        help=(
            "Click the publish button after a successful form fill. This is "
            "the only mode that publishes."
        ),
    )
    ...
    publish_mode_group.add_argument(
        ...
        default=False,
        help=(
            "Explicit preview mode: fill the form and never click publish "
            "(also the default when neither mode flag is supplied)."
        ),
    )
    ...
    weighted_title_length = _weighted_title_length(title)
    if weighted_title_length > MAX_WEIGHTED_TITLE_LENGTH:
        print(
            "Error: title is too long "
            f"({weighted_title_length}/{MAX_WEIGHTED_TITLE_LENGTH} weighted characters).",
            file=sys.stderr,
        )
        sys.exit(2)
    ...
    # --- Step 5: Publish only after explicit opt-in ---
    should_publish = _should_publish(args.auto_publish, args.preview)
    if should_publish:
        print("[pipeline] Auto-publish mode explicitly enabled.")
    elif args.preview:
        print("[pipeline] Explicit preview mode: skipping publish click.")
    else:
        print(
            "[pipeline] Safe default preview mode: no publish mode flag "
            "supplied; skipping publish click."
        )
